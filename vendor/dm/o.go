...
func (conn *DmConnection) setTrxFinish(status int32) {
	switch status & Dm_build_784 {
	case Dm_build_781, Dm_build_782, Dm_build_783:
		conn.trxFinish = true
	default:
...
func (dc *DmConnection) executeInner(query string, execType int16) (interface{}, error) {
	...
	if execType == Dm_build_748 && dc.dmConnector.enRsCache {
	...
	if optParamList != nil && len(optParamList) > 0 {
		info, err = dc.Access.Dm_build_488(stmt, optParamList)
		if err != nil {
			stmt.nativeSql = query
			info, err = dc.Access.Dm_build_494(stmt, execType)
		}
	} else {
		info, err = dc.Access.Dm_build_494(stmt, execType)
	}
...
func (dc *DmConnection) reconnect() error {
	...
	for _, stmt := range dc.stmtMap {
		err = dc.Access.Dm_build_466(stmt)
		if err != nil {
			return err
		}
...
func (dc *DmConnection) close() error {
	...
	if dc.stmtPool != nil {
		for _, spi := range dc.stmtPool {
			err = dc.Access.Dm_build_471(spi.id)
			if err != nil {
				return err
			}
		}
...
func (dc *DmConnection) exec(query string, args []driver.Value) (*DmResult, error) {
	...
	if args != nil && len(args) > 0 {
		...
		r1, err := dc.executeInner(query, Dm_build_749)
		if err != nil {
			return nil, err
		}
...
func (dc *DmConnection) query(query string, args []driver.Value) (*DmRows, error) {
	...
	if args != nil && len(args) > 0 {
		...
		r1, err := dc.executeInner(query, Dm_build_748)
		if err != nil {
			return nil, err
		}
